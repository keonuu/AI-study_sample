...
6. 오답이나 교정이 필요한 부분은 `[오답/교정]` 라벨을 붙이고, 가능하면 GitHub Markdown alert 형식인 `> [!CAUTION]` block으로 표시한다. inline HTML 색상 표시는 사용하지 않는다.
7. 필요하면 꼬리 질문이나 추가 예제를 낸다.
8. 이해가 확인되면 다음 개념으로 넘어간다.
```
