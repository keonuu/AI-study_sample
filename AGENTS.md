...
문제 풀이 피드백에서 사용자의 답이 틀렸거나 표현을 교정해야 할 때는 오답/교정 부분을 눈에 띄게 표시한다.

기본 형식은 GitHub Markdown alert/callout 형식을 우선한다.

```markdown
> [!CAUTION]
> [오답/교정] 여기에 틀린 부분과 정확한 교정 설명을 쓴다.
```

규칙:

1. 맞은 부분, 틀린 부분, 애매한 부분을 분리해서 채점한다.
2. 틀린 부분 또는 교정이 필요한 설명만 `[!CAUTION]` block에 넣는다. 전체 답변을 불필요하게 전부 경고 block으로 만들지 않는다.
3. 렌더러마다 색상 지원이 다르므로, 색상에만 의존하지 말고 항상 `[오답/교정]` 라벨을 함께 붙인다.
4. inline HTML `<span style="color:red">...</span>`는 채팅/Markdown 렌더러에서 그대로 노출될 수 있으므로 사용하지 않는다.
5. 저장되는 Day 파일의 Correction Points에도 같은 원칙을 적용하되, 나중에 검색 가능한 `[오답/교정]` 라벨을 우선한다.
