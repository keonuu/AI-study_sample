...
```html
<span style="color:red">[오답/교정] 여기에 틀린 부분과 정확한 교정 설명을 쓴다.</span>
```

규칙:

1. 맞은 부분, 틀린 부분, 애매한 부분을 분리해서 채점한다.
2. 틀린 부분 또는 교정이 필요한 설명만 빨간색으로 표시한다. 전체 답변을 불필요하게 전부 빨갛게 만들지 않는다.
3. 렌더러가 HTML 색상을 지원하지 않을 수 있으므로, 항상 `[오답/교정]` 라벨도 함께 붙인다.
4. 저장되는 Day 파일의 Correction Points에도 같은 원칙을 적용하되, 색상 표시보다 나중에 검색 가능한 `[오답/교정]` 라벨을 우선한다.
